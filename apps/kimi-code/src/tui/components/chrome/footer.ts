 ...
 *
 * Layout:
 *   Line 1: [yolo] [plan] <model> <usage%> <cwd>  <git-badge>  <shortcut hints>
 *   Line 2: [transient hint | first quota row | custom status line]
 *             context: N% (tokens/max)
 ...
 * line 3 with a refresh stamp. When a `status_line.command` is set and there
 * is no quota data, the command's first stdout line fills line 2's left slot.
 * The transient hint takes precedence over both while visible.
 */
...
export class FooterComponent implements Component {
  ...
  render(width: number): string[] {
    ...
    if (this.transientHint) {
      ...
      // otherwise a configured custom status line fills it.
      const content =
        usage !== null ? this.renderUsageRow(usage.rows[0]!, usage.labelWidth) : customLine;
      if (content === null) {
